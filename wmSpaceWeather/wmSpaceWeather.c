 ...
 *      Changes:
 *
 *      Version 1.05 - release May, 20, 2024 (Chris Ringeval)
 *
 *                     Making it work again, hoping to not miss the
 *                     next solar storm in 20 years from now... My
 *                     perl sucks, moved to python and json
 *                     requests. Kp are now real numbers, some tuning
 *                     for the e- and p+ spectra, the X-ray reports latest
 *                     flares. All this decided in the getspaceweather.py script.
 *
 ...
 */
#define DELAY 10000L
#define WMSPACEWEATHER_VERSION "1.05"
...
FILE		*fp;
	...
	if (dt2 > 2){
	    ...
	    CurrentJD = jd(Year, Month, Day, UT);
    	    ...
    	    if ((fp = fopen("/tmp/LatestKp.txt", "r")) != NULL){
    	        ...
    	        for (i=0; i<8; ++i){
	    	    fscanf(fp, "%ld %lf", &TimeTag[i], &Kp[i]);
	...
	if (((!UpToDate)&&(dt1 > 300))||ForceUpdate2){
	    ...
	    dt1 = 0;
    	    ...
    	    /*
    	     *  Execute Python script to retrieve the Latest Kp values and compute spectra
    	     */
    	    system("getspaceweather.py &");
